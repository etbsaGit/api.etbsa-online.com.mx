...
namespace App\Http\Requests\Intranet\Classifications;
...
class ClassificationRequest extends FormRequest
{
    ...
    {
        $item = $this->route('classification');
        return [
            'name' => ['required', 'string', 'max:191', Rule::unique('classifications', 'name')->ignore($item?->id)],
        ];
    }
